...
class model_cfg_bank(object):
    ...
    def get_yaml_path(self, name):
        ...
        elif name.find('shgan')==0:
            return osp.join(
                self.cfg_dir, 'shgan.yaml')
        else:
            raise ValueError
...
def get_command_line_args():
    ...
    parser.add_argument('--evalnog_path', type=str)
    ...
    if args.evalnog_path is not None:
        if 'eval' in cfg:
            cfg.eval.dataset.gen_dir = args.evalnog_path
...
def cfg_initiates(cfg):
    ...
    if isresume:
        ...
        if cfg.env.gpu_device != 'all':
            os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(
                [str(gid) for gid in cfg.env.gpu_device]) 

        if cfg.train.save_code:
            codedir = osp.join(cfg.train.log_dir, 'code')
