...
def profile(module, input, nb_iters=1000):
    ...
    for _ in range(nb_iters):
        ckpt = time.time()
        output = module(input)
        ckpt_fwd = time.time() - ckpt; ckpt = time.time()
        output.sum().backward()
        ckpt_bwd = time.time() - ckpt; ckpt = time.time()
        for param in module.parameters():
            param.grad = None
    print(ckpt_fwd / nb_iters, ckpt_bwd / nb_iters)

def profile2(module, input):
    try:
        input = [input.detach()]
    except:
        input = [i.detach() for i in input]
